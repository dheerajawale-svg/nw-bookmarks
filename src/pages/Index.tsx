        ...
        <main>
          <section className="mb-8">
            ...
            <div className="space-y-4">
              {bookmarks.map(bookmark => <BookmarkCard key={bookmark.id} title={bookmark.title} reference={bookmark.reference} timeRange={bookmark.timeRange} content={bookmark.content} filters={bookmark.filters} userInitials={bookmark.userInitials} isEditable={bookmark.state !== 'disabled'} state={getBookmarkState(bookmark)} onContentChange={newContent => handleContentChange(bookmark.id, newContent)} onMenuClick={() => handleMenuClick(bookmark.id)} onNoteClick={() => handleNoteClick(bookmark.id)} onClick={() => handleCardClick(bookmark.id)} />)}
            </div>
          </section>
