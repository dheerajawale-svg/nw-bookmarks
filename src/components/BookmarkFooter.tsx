...
export const BookmarkFooter: React.FC<BookmarkFooterProps> = ({
  ...
  return (
    <>
      <Divider sx={{ width: '100%', borderColor: 'hsl(var(--card-normal-border, 0 0% 91.8%))' }} />
      <Box
        component="footer"
        sx={{
          display: "flex",
      ...
      >
        ...
        >
          {filters.map((filter, index) => (
            <FilterIcon key={index} type={filter.type} value={filter.value} className="flex-shrink-0" />
          ))}
        </Box>
        <Box sx={{ display: "flex", width: 24, height: 24, alignItems: "center", gap: 2 }}>
          <IconButton
          ...
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: 16, height: 16 }}>
              <path
